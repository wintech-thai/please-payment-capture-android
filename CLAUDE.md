...
- Duplicate notifications (rapid *updates* ~1–2 s apart, and the same payment re-posted under a different `notification.key` such as a group summary + child) are filtered **in-memory** by `NotificationCaptureService` using a `processedCache` keyed on `packageName:title:text` (**not** `notification.key` — including it would let same-content/different-key posts double-forward to the bank endpoint) with a **2-second** window.
